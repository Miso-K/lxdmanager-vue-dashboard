...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      projects() {
        return this.$store.getters.projectsTableData;
      }
    },
    methods: {
      getKeys(item) {
        ...
        setTimeout(() => {
          try {
            ...
            if (this.editedIndex === -1) {
              this.$store.dispatch('createProject', { data: yaml.safeLoad(this.yamlString) });
            } else {
              this.$store.dispatch('updateProject', { data: yaml.safeLoad(this.yamlString) });
            }
            setTimeout(() => {
              this.$store.dispatch('fetchProjects');
            }, 1000);
      ...
      deleteProject() {
        this.$store.dispatch('deleteProject', this.editedName);
        setTimeout(() => {
