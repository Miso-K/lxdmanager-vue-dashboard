      ...
      <v-data-iterator :items="containers" :rows-per-page-items="rowsPerPageItems" :hide-actions="total < 8" :pagination.sync="pagination" content-tag="v-layout" row wrap>
        <v-flex slot="item" slot-scope="props" xs12 sm6 md4 lg3>
          ...
          >
              ...
              <v-spacer></v-spacer>
              <v-btn color="primary" small left :to="'/machine/'+props.item.id">Manage</v-btn>

          </v-toolbar>
            <v-divider></v-divider>
            <v-list dense>
              <v-list-tile>
                <v-list-tile-content>Memory:</v-list-tile-content>
              ...
              <v-list-tile>
                <v-list-tile-content>Disk:</v-list-tile-content>
                <v-list-tile-content class="align-end pa-1">
                  {{ usage(40000, 20000) }}
                </v-list-tile-content>
                <v-list-tile-content class="align-end pa-1">
                  <v-progress-circular
                    ...
                    :value="percentage(props.item.config.limits_disk_raw, 20)"
                  >
                    {{ percentage(props.item.config.limits_disk_raw, 20) }}%
                  </v-progress-circular>
                </v-list-tile-content>
              </v-list-tile>
              <v-list-tile>
